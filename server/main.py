...
class HandChronometer:
    ...
    def process_hand_action(self, finger_count, current_time, pose_landmarks, new_height, new_width):
        if current_time - self.last_action_time < self.action_cooldown:
            return False

        if finger_count - 1 == 2:  # Two fingers up
            if not self.extrema_tracking['active']:
                self.extrema_tracking['active'] = True
                self.extrema_tracking['start_time'] = current_time
                self.extrema_tracking['max_y'] = float('-inf')
                self.extrema_tracking['min_y'] = float('inf')
                self.chronometer.start()
            
            # Track wrist extrema during 4-second window
            if pose_landmarks:
            ...
            # Stop tracking after 4 seconds
            if current_time - self.extrema_tracking['start_time'] >= 4:
                self.chronometer.stop()
                self.extrema_tracking['active'] = False
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            time_text = f"Time: {self.chronometer.get_time():.2f} sec"
            ...
            if hand_result.multi_hand_landmarks:
                ...
                for hand_landmarks, hand_handedness in zip(hand_result.multi_hand_landmarks, hand_result.multi_handedness):
                    ...
                    self.process_hand_action(finger_count, current_time, pose_result.pose_landmarks, new_height, new_width)
